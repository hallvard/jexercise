...
entryRuleSuiteInstance returns [EObject current=null] 
	:
	{ newCompositeNode(grammarAccess.getSuiteInstanceRule()); }
	 iv_ruleSuiteInstance=ruleSuiteInstance 
	 { $current=$iv_ruleSuiteInstance.current; } 
	 EOF 
;

// Rule SuiteInstance
ruleSuiteInstance returns [EObject current=null] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
((
		...
		lv_parameterTypes_3_0=ruleParameterType		{
	        if ($current==null) {
       		...
       		add(
        		...
        		"no.hal.jex.jextest.JexTest.ParameterType");
	        afterParserOrEnumRuleCall();
	    }
