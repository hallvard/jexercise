...
public class EmfsContainerItemProvider extends EmfsResourceItemProvider {
	...
	protected Command createDragAndDropCommand(EditingDomain domain, Object owner, float location, int operations, int operation, Collection<?> collection) {
		...
		// only do it, if the EmfsResource structure is the main hierarchy in this resource
		if (owner instanceof EmfsContainer && Util.isMainEObject(EcoreUtil.getRootContainer((EObject) owner)) && resources.size() > 0) {
			return new ImportEmfsResourcesCommand((EmfsContainer) owner, resources);
		}
