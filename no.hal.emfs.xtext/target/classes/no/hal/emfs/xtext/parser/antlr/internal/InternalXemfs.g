...
// Entry rule entryRuleGIT_HOST
entryRuleGIT_HOST returns [String current=null] 
	:
	{ newCompositeNode(grammarAccess.getGIT_HOSTRule()); } 
	 iv_ruleGIT_HOST=ruleGIT_HOST 
	 { $current=$iv_ruleGIT_HOST.current.getText(); }  
	 EOF 
;

// Rule GIT_HOST
ruleGIT_HOST returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
(    this_ID_0=RULE_ID    {
